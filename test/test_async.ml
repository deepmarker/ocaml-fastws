...
let connect_f mv w =
  ...
  write_frame_if_open w msg >>= fun () ->
  Mvar.take mv >>= fun { header; payload } ->
  write_frame_if_open w (Frame.Bigstring.close ()) >>= fun () ->
  Mvar.take mv >>| fun _cl ->
  let msg' =
    {
      Frame.header;
      payload = Option.value ~default:(Bigstringaf.create 0) payload;
    }
  in
  check frame "" msg msg'
...
let connect () =
  ...
  Fastws_async.Raw.(to_or_error (connect url r w)) >>=? fun (_exts, r, w) ->
  Deferred.all_unit
