...
let frames =
  [
    ...
    ("close", closef "bleh");
    ( "unfinished cont",
      { header = create ~final:false Continuation; payload = None } );
    ( "text with rsv",
      { header = create ~final:false ~rsv:7 Text; payload = None } );
...
let roundtrip ?mask descr frames () =
  ...
  let rec inner acc pos =
    if pos = len then List.rev acc
      ...
      | `Ok (t, nb_read) -> (
          match t.length with
          | 0 -> inner ({ header = t; payload = None } :: acc) (pos + nb_read)
          | len ->
