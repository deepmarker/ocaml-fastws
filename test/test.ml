...
let frames =
  ...
  [ ("empty text", textf ""); ("text with content", textf "test");
    ("empty continution", createf Continuation ""); ("ping", createf Ping "");
    ("pong", createf Pong ""); ("empty close", close ());
    ("close", closef "bleh");
    ( "unfinished cont",
    ...
    if pos = len then List.rev acc
      ...
      | `Ok (t, nb_read) -> (
        ...
        | len ->
            ...
            inner ({header= t; payload} :: acc) (pos + nb_read + len) ) ) in
...
let () =
  run "fastws"
    [ ("roundtrip", roundtrip_unmasked); ("roundtrip_masked", roundtrip_masked);
