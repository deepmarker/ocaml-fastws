...
let decompress st msg =
  ...
  let rec loop added_trailer =
    ...
    | Data_error msg -> failwithf "zlib: %s" msg ()
  in
  (* entry point *)
...
let reassemble st (t : Fastws_async_raw.t) =
  ...
  (* Erroneous cases *)
  | { header = { opcode; final = false; _ }; _ }, _
  ...
  | { header = { opcode = Continuation; final; _ }; payload }, Some h ->
      if final then (
        ...
        let payload =
          if must_decompress st h then decompress st payload else payload
        in
        let length = Bigstring.length payload in
        Ok
          (`Frame
            { Frame.header = { h with final = true; rsv = 0; length }; payload }))
      else (
        Option.iter payload ~f:(Bigbuffer.add_bigstring st.buf);
        Ok `Continue)
...
let mk_r3 ?monitor of_frame st r2 w2 =
  ...
  let transferf q =
    ...
    | Error exn ->
        write_close st w2
          (Frame.String.close ~status:(Status.UnsupportedDataType, None) ())
        >>| fun () ->
        close_all ();
...
let connect_or_error ?timeout ?on_pong ?extra_headers ?extensions ?protocols
    ?monitor ?hb of_frame to_frame url =
  let f () =
    Async_uri.connect url >>= fun { r; w; _ } ->
    Monitor.try_with ~extract_exn:true (fun () ->
        connect ?on_pong ?extra_headers ?extensions ?protocols ?monitor ?hb url
          r w of_frame to_frame)
    >>= function
    | Error exn -> Writer.close w >>= fun () -> raise exn
  ...
  | None -> f ()
  | Some timeout -> (
      Clock_ns.with_timeout timeout (f ()) >>| function
